...
const renderApcoMasterLayout = async (
  ...
  if (!logoToUse) {
     ...
     const brandNameText = (settings.companyName || 'Studio').toUpperCase();
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(brandPrimary[0], brandPrimary[1], brandPrimary[2]);
  const brandCursive = settings.companyName || 'Studio';
...
export const generateInvoicePDF = async (invoice: Invoice, client: Client, settings: CompanyProfile, autoSave = true) => {
  ...
  const templateId = (docTemplateId && registry[docTemplateId]) ? docTemplateId : companyTemplateId;
