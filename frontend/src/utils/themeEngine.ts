...
export const applyOrgBranding = (org: { 
  ...
  const secondary = org.secondaryColor || '#6F6A61';
  const accent = org.accentColor || '#B89452';
  const isDark = org.themePreset === 'dark';

  const bg = org.backgroundColor || (isDark ? '#0c0b0a' : '#FAF9F6');
  const text = org.textColor || (isDark ? '#fdfbf7' : '#1c1a17');

  root.style.setProperty('--primary-color', accent);
  root.style.setProperty('--theme-primary', accent);
  root.style.setProperty('--primary-color-rgb', hexToRgb(accent));
  
  if (isDark) {
    document.body.dataset.themeType = 'dark';
    root.setAttribute('data-theme-type', 'dark');
    root.style.setProperty('--theme-bg', bg);
    root.style.setProperty('--theme-card-bg', 'rgba(255, 255, 255, 0.02)');
    root.style.setProperty('--theme-border', 'rgba(255, 255, 255, 0.08)');
    root.style.setProperty('--theme-text', text);
    root.style.setProperty('--theme-text-muted', secondary);
    document.body.classList.add('dark');
  } else {
    document.body.dataset.themeType = 'light';
    root.setAttribute('data-theme-type', 'light');
    root.style.setProperty('--theme-bg', bg);
    root.style.setProperty('--theme-card-bg', '#ffffff');
    root.style.setProperty('--theme-border', '#E7E2D9'); // #E7E2D9 from premium design guidelines
    root.style.setProperty('--theme-text', text);
    root.style.setProperty('--theme-text-muted', secondary);
    document.body.classList.remove('dark');
  }

  // Load Typography Preset (Font Safety Allowlist)
  const fontPreset = org.typographyPreset || 'CLASSIC';
  const fonts = {
    CLASSIC: { family: '"Playfair Display", Georgia, serif', url: 'https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700&display=swap' },
    MODERN: { family: '"Space Grotesk", sans-serif', url: 'https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&display=swap' },
    ELEGANT: { family: '"Cormorant Garamond", serif', url: 'https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400&display=swap' },
    EDITORIAL: { family: '"DM Serif Display", serif', url: 'https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&display=swap' },
    MINIMAL: { family: '"Inter", sans-serif', url: 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap' },
  };

  const selectedFont = fonts[fontPreset as keyof typeof fonts] || fonts.CLASSIC;
  
  if (selectedFont.url) {
    const linkId = `font-preset-${fontPreset}`;
    if (!document.getElementById(linkId)) {
      const link = document.createElement('link');
      link.id = linkId;
      link.rel = 'stylesheet';
  ...
  root.style.setProperty('--theme-font-family', selectedFont.family);
  ...
  favicon.href = org.faviconUrl || '/favicon.ico';

  // Update document title
  document.title = `${org.name} | Studio`;
};
