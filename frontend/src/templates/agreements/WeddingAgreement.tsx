...
export const WeddingAgreement: React.FC<TemplateProps> = ({ company, client, document, agreement }) => {
  ...
  const defaultTerms = [
    ...
    },
    {
      ...
      bullets: [
        { label: "", text: "The works may be used for the portfolios, websites, or social media promotions of the Service Provider or any of its subsidiaries. This creative promotional right remains with the Service Provider for all time and across any period in the future." },
  ...
  return (
    <div className="bg-white text-[#111111] w-full max-w-4xl mx-auto border border-[#EAEAEA] rounded-none shadow-none font-sans p-8 md:p-14 space-y-12 selection:bg-amber-100">
          ...
          <div className="flex items-center gap-4">
            {company?.logo ? (
              ...
              <span className="font-serif text-xl font-bold tracking-wider text-zinc-900">
                {company?.companyName || 'STUDIO WORKSPACE'}
              </span>
            )}
          </div>
          {company?.tagline && (
            <div className="text-right">
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{company.tagline}</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
          ...
          <div className="border border-[#EAEAEA] p-6 rounded-none bg-white space-y-3">
            ...
            <div className="text-sm space-y-1.5 leading-[1.6] text-zinc-800">
              ...
              <p><span className="font-semibold text-zinc-900">Legal Entity:</span> {company?.companyName || 'Studio'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-6 border-t border-[#EAEAEA]">
          ...
          <div className="space-y-6 flex flex-col justify-between h-full min-h-[220px]">
            ...
            <div className="text-xs space-y-1 text-zinc-800 pt-2 border-t border-zinc-200">
              ...
              <p className="font-bold text-zinc-900 uppercase tracking-wide">{company?.companyName || 'Studio Workspace'}</p>
              {company?.tagline && <p className="text-[11px] text-zinc-500 uppercase tracking-wider font-medium">{company.tagline}</p>}
            </div>
          </div>
      ...
      <div className="pt-8 mt-12 border-t border-[#EAEAEA] text-[11px] text-zinc-500 font-sans flex flex-col md:flex-row justify-between items-center gap-3">
        <div>
          <span className="font-semibold text-zinc-900">{company?.companyName || 'Studio Workspace'}</span> {company?.tagline ? ` • ${company.tagline}` : ''}
        </div>
        <div className="text-center">
          {company?.website} {company?.email ? ` • ${company.email}` : ''} {company?.phone ? ` • ${company.phone}` : ''}
        </div>
