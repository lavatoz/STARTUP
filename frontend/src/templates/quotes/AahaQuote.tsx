    ...
    <div className="bg-[#FAF9F6] text-amber-950 p-16 min-h-[800px] w-full max-w-4xl mx-auto font-serif relative shadow-xl">
      ...
      <div className="text-center mb-16 relative z-10">
         {company?.logo ? (
            ...
            <div className="w-20 h-20 mx-auto rounded-full bg-amber-50 border-2 border-amber-200 flex items-center justify-center mb-6">
               <span className="text-2xl text-amber-700 font-bold">{company?.companyName?.charAt(0) || 'S'}</span>
            </div>
         )}
         <h1 className="text-4xl font-bold tracking-widest uppercase text-amber-900">{company?.companyName || 'Studio'}</h1>
         <p className="text-sm italic text-amber-700/70 mt-2 font-light">{company?.tagline || 'Timeless Stories Captured'}</p>
      ...
      <div className="mt-auto text-center px-4">
         ...
         <div className="mt-8 flex justify-center gap-6 text-[10px] uppercase tracking-widest text-amber-900/40">
            ...
            <span>•</span>
            <span>{company?.phone || company?.website || 'www.studio.com'}</span>
         </div>
      </div>
    </div>
  );
