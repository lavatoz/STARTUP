...
import { applyOrgBranding } from './utils/themeEngine';
...
const App: React.FC = () => {
  ...
  // Load memberships and resolve active organization
  useEffect(() => {
    const loadMemberships = async () => {
      ...
      try {
        const orgs = await api.getMemberships();
        setMemberships(orgs);
        
        if (orgs.length > 0) {
          const storedId = localStorage.getItem('active_org_id');
          const matched = orgs.find((o: any) => o.id === storedId);
          const current = matched || orgs[0];
          
          setActiveOrg(current);
          localStorage.setItem('active_org_id', current.id);
          localStorage.setItem('active_org_slug', current.slug);
          applyOrgBranding(current);
        } else {
          setActiveOrg(null);
          const path = window.location.pathname;
          const isPublic = ['/login', '/register', '/forgot-password', '/reset-password', '/onboarding'].some(p => path.startsWith(p));
          if (!isPublic) {
            navigate('/onboarding');
          }
        }
  ...
  const handleSwitchOrganization = (orgId: string, orgSlug: string) => {
    localStorage.setItem('active_org_id', orgId);
    localStorage.setItem('active_org_slug', orgSlug);
    
    const selected = memberships.find(m => m.id === orgId);
    if (selected) {
      setActiveOrg(selected);
      applyOrgBranding(selected);
    }
    
    // Clear list states to avoid flash of stale data
    setClients([]);
    setProjects([]);
    setInvoices([]);
    setTasks([]);
  ...
  // Guard for users with no organizations or when explicitly on /onboarding
  if (!activeOrg || window.location.pathname === '/onboarding') {
    return (
      <Routes>
