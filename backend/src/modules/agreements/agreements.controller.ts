...
import { getOrCreateProjectFolderStructure, uploadFile } from '../../services/google-drive.service';
...
export async function generateProjectAgreement(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // 2. Fetch Project & Client in org context
    ...
    // 3. Fetch Quotation & Invoice information for placeholders in org context
    ...
    // 4. Fetch successful payments logged for project in org context
    ...
    // 5. Find linked events for placeholder names/dates in org context
    const taskWithEvent = await prisma.task.findFirst({
      where: { projectId, organizationId: orgId, eventId: { not: null } },
      include: { event: true },
    });
    ...
    if (!defaultEventName) {
      const firstEvent = await prisma.event.findFirst({
        where: { clientId: project.clientId, organizationId: orgId },
        orderBy: { date: 'asc' },
      });
    ...
    // Resolve active brand profile and logo in org context
    let brandProfile = null;
    if (quotation?.brandId) {
      brandProfile = await prisma.companyProfile.findFirst({
        where: { id: quotation.brandId, organizationId: orgId, deletedAt: null },
      });
    }

    if (!brandProfile && quotation?.brand) {
      brandProfile = await prisma.companyProfile.findFirst({
        where: { companyName: quotation.brand, organizationId: orgId, deletedAt: null },
      });
    }
    // Fetch default company profile in org context
    const companyProfile = await prisma.companyProfile.findFirst({
      where: { isDefault: true, organizationId: orgId, deletedAt: null },
    });
    ...
    // Resolve client event for venue details in org context
    ...
    // 8. Generate atomic counter-based AGR agreement number: AGR-{YEAR}-{SEQUENCE} in org context
    ...
    const counterResult = await prisma.$queryRaw<Array<{ lastValue: number }>>`
      ...
      VALUES (${orgId}, 'AGR', 'AGR_SEQ', ${agrYear}, 1)
    ...
    // Generate Document ID and Verification URL for the Document Registry in org context
    ...
    // 7. Store physical copy locally
    const outputDir = path.join(__dirname, '../../../files/agreements');
    ...
    const relativeLocalPath = `files/agreements/${fileName}`;

    // Upload to Google Drive (if folders exist or configured)
    ...
    try {
      ...
      const agreementsFolderId = folderStructure?.agreementsFolderId;
      if (folderStructure && agreementsFolderId !== project.agreementsFolderId) {
        await prisma.project.update({
          where: { id: projectId, organizationId: orgId },
          data: { agreementsFolderId },
        });
      }
    ...
    // Register document in the Document Registry in org context
