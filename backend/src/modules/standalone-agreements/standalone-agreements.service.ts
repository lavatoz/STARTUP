...
export class StandaloneAgreementsService {
   ...
   */
  static async getTemplates(organizationId: string): Promise<StandaloneAgreementTemplate[]> {
   ...
   */
  static async getTemplateById(organizationId: string, id: string): Promise<StandaloneAgreementTemplate> {
    const template = await prisma.standaloneAgreementTemplate.findFirst({
  ...
  static async updateTemplate(
    ...
    const existing = await prisma.standaloneAgreementTemplate.findFirst({
      where: { id, organizationId },
    });
    if (!existing) {
      throw new AppError('Standalone agreement template not found.', 404);
   ...
   */
  static async deleteTemplate(organizationId: string, id: string): Promise<StandaloneAgreementTemplate> {
    const existing = await prisma.standaloneAgreementTemplate.findFirst({
      where: { id, organizationId },
    });
    if (!existing) {
   ...
   */
  static async assignAgreement(organizationId: string, clientId: string, templateId: string, linkedQuoteId?: string | null): Promise<StandaloneAgreement> {
   ...
   */
  static async getClientAgreement(organizationId: string, clientId: string): Promise<any[]> {
    const client = await prisma.client.findFirst({
      where: { id: clientId, organizationId, deletedAt: null },
    });
    if (!client) {
  ...
  static async uploadDocument(
    ...
    const agreement = await prisma.standaloneAgreement.findFirst({
   ...
   */
  static async getDocuments(organizationId: string, agreementId: string): Promise<StandaloneAgreementDocument[]> {
    const agreement = await prisma.standaloneAgreement.findFirst({
      where: { id: agreementId, organizationId },
    });
    if (!agreement) {
   ...
   */
  static async deleteDocument(organizationId: string, documentId: string): Promise<StandaloneAgreementDocument> {
    const doc = await prisma.standaloneAgreementDocument.findFirst({
      where: {
        id: documentId,
  ...
  static async getDocumentForDownload(
    ...
    if (user.role === Role.Client) {
      const client = await prisma.client.findFirst({
        where: { email: user.email, organizationId, deletedAt: null },
      });
   ...
   */
  static async generateSignedAgreementPdf(organizationId: string, agreementId: string): Promise<string> {
   ...
   */
  static async linkQuotation(organizationId: string, agreementId: string, linkedQuoteId: string): Promise<StandaloneAgreement> {
    // 1. Fetch agreement
    const agreement = await prisma.standaloneAgreement.findFirst({
      where: { id: agreementId, organizationId },
    });
    ...
    const quotation = await prisma.quotation.findFirst({
      where: { id: linkedQuoteId, organizationId, deletedAt: null },
    });
    if (!quotation) {
   ...
   */
  static async acceptQuotation(organizationId: string, quotationId: string): Promise<StandaloneAgreement> {
    const quotation = await prisma.quotation.findFirst({
    ...
    let existingInvoice = await prisma.invoice.findFirst({
      where: { projectId: quotation.projectId, organizationId, deletedAt: null },
    });
    ...
    if (quotation.templateId) {
      template = await prisma.standaloneAgreementTemplate.findFirst({
        where: { id: quotation.templateId, organizationId },
      });
    }
   ...
   */
  static async getClientAgreementsList(organizationId: string, user: { id: string; email: string; role: Role }, clientIdParam?: string): Promise<any[]> {
    ...
    for (const agr of agreements) {
      ...
      if (agr.linkedQuoteId) {
        const quotation = await prisma.quotation.findFirst({ where: { id: agr.linkedQuoteId, organizationId } });
        if (quotation?.brand) {
   ...
   */
  static async getClientAgreementDetails(organizationId: string, agreementId: string, user: { id: string; email: string; role: Role }): Promise<any> {
    const agreement = await prisma.standaloneAgreement.findFirst({
      where: { id: agreementId, organizationId },
      include: {
        template: true,
    ...
    if (agreement.linkedQuoteId) {
      quotation = await prisma.quotation.findFirst({
        where: { id: agreement.linkedQuoteId, organizationId },
        include: { items: true },
      });
    }
