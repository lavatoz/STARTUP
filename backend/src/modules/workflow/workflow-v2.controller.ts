 ...
 */
async function verifyProjectAccess(orgId: string, projectId: string, user: { id: string; role: Role; email: string }) {
...
export async function getProjectWorkflow(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    await verifyProjectAccess(orgId, projectId, user);
...
export async function updateProjectWorkflowStage(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // 1. Verify project access
    const project = await verifyProjectAccess(orgId, projectId, user);
