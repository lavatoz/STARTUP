...
import { Role, OrganizationRole, SubscriptionStatus } from '@prisma/client';
...
export async function login(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const formattedMemberships = memberships.map(m => ({
      ...
      themePreset: m.organization.themePreset,
    }));
...
/**
 * Endpoint: Onboard Organization & Initialize Workspace
 * POST /auth/onboard
 */
export async function onboard(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!dbUser.emailVerified) {
      throw new AppError('You must verify your email address before onboarding.', 403);
    }

    const existingMembershipsCount = await prisma.organizationMember.count({
      where: { userId: user.id },
    });
    if (existingMembershipsCount >= 5) {
      throw new AppError('You have reached the limit of organizations you can create.', 400);
    }

    const slug = body.slug.toLowerCase().trim();
    const reservedSlugs = [
      'admin', 'api', 'platform', 'login', 'register', 'settings', 
      'support', 'billing', 'app', 'system', 'public', 'auth', 
      'gallery', 'workflow', 'notifications', 'tasks', 'approvals', 'personnel'
    ];
    if (reservedSlugs.includes(slug)) {
      throw new AppError('This studio slug is reserved. Please choose another one.', 400);
    }
    ...
    if (existingOrg) {
      throw new AppError('This studio slug is already in use.', 400);
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      const org = await tx.organization.create({
        data: {
          ...
          primaryColor: body.primaryColor || '#F7F5F0',
          secondaryColor: body.secondaryColor || '#77736B',
          accentColor: body.accentColor || '#B89452',
          themePreset: body.themePreset || 'light',
          businessDesc: body.businessDesc || null,
          contactEmail: body.contactEmail || dbUser.email,
      ...
      await tx.companyProfile.create({
        data: {
          ...
          email: org.contactEmail || dbUser.email,
          phone: org.phone || '',
          address: org.address || '',
          ...
          portalConfig: {
            secondaryColor: org.secondaryColor || '#6F6A61',
            accentColor: org.accentColor || '#B89452',
      ...
      await tx.agreementTemplate.create({
        data: {
          ...
          body: 'This agreement constitutes a binding contract for photography services...',
          version: 1
        }
      });
    ...
    await logAudit({
      ...
      details: { organizationId: result.org.id, name: result.org.name, slug: result.org.slug },
      ...meta,
    });
...
/**
 * Endpoint: List User's Active Memberships
