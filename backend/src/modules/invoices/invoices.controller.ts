 ...
 */
async function resolveCompanyPrefix(organizationId: string, projectId: string): Promise<string> {
  ...
  if (project) {
    const company = await prisma.companyProfile.findFirst({
      where: { organizationId, projectType: project.status || '', deletedAt: null }, // Mock mapping
    });
  ...
  return defaultCompany?.invoicePrefix || 'STUDIO';
}
...
export async function getInvoiceById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (user.role === Role.Client) {
      const client = await prisma.client.findFirst({ where: { email: user.email, organizationId: orgId, deletedAt: null } });
...
export async function createInvoice(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const project = await prisma.project.findFirst({ where: { id: projectId, organizationId: orgId, deletedAt: null } });
    ...
    const prefix = await resolveCompanyPrefix(orgId, projectId);
...
export async function updateInvoice(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const existing = await prisma.invoice.findFirst({ where: { id, organizationId: orgId, deletedAt: null } });
...
export async function updateQuotation(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const existing = await prisma.quotation.findFirst({ where: { id, organizationId: orgId, deletedAt: null } });
  ...
  try {
    ...
    if (quotation.brandId) {
      brandProfile = await prisma.companyProfile.findFirst({
        where: { id: quotation.brandId, organizationId: user.organizationId!, deletedAt: null },
      });
    }

    if (!brandProfile && quotation.brand) {
      brandProfile = await prisma.companyProfile.findFirst({
        where: { companyName: quotation.brand, organizationId: user.organizationId!, deletedAt: null },
      });
    }
    ...
    const companyProfile = await prisma.companyProfile.findFirst({
      where: { isDefault: true, organizationId: user.organizationId!, deletedAt: null },
    });
    ...
    const taskWithEvent = await prisma.task.findFirst({
      where: { projectId: quotation.projectId, organizationId: user.organizationId!, eventId: { not: null } },
      include: { event: true },
    });
    ...
    const clientEvents = await prisma.event.findMany({
      where: { clientId: quotation.clientId, organizationId: user.organizationId! },
      orderBy: { date: 'asc' },
    });
    ...
    const pdfBuffer = await generateQuotationPdf({
      ...
      companyName: companyProfile?.companyName || 'Studio',
      bankDetails,
      companyLogoUrl,
    ...
    try {
      ...
      if (folderStructure.quotationsFolderId !== quotation.project.quotationsFolderId) {
        await prisma.project.update({
          where: { id: quotation.projectId, organizationId: user.organizationId! },
          data: {
    ...
    const fileRecord = existingFile
      ? await prisma.file.update({
          where: { id: existingFile.id, organizationId: user.organizationId! },
          data: {
