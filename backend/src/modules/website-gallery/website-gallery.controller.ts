...
export async function uploadWebsiteGalleryCover(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // 8. Upload, configure public sharing, generate URL, and verify accessibility
    let fileId: string;
    let imageUrl: string;
    try {
...
export async function getWebsiteGalleryById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const { id } = req.params;
    const gallery = await prisma.websiteGallery.findFirst({
...
export async function updateWebsiteGallery(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Check existence in org context
    const existing = await prisma.websiteGallery.findFirst({
      where: { id, organizationId: orgId },
    });
    if (!existing) {
      throw new AppError('Website gallery item not found.', 404);
 ...
 */
export async function getPublicWebsiteGalleries(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
