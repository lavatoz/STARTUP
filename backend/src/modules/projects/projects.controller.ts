...
export async function updateProject(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Admin/Manager only
    if (user.role !== Role.SystemAdmin && user.role !== Role.Manager) {
      throw new AppError('Only administrators or managers can modify projects.', 403);
    }
    ...
    if (body.status && body.status !== project.status) {
      ...
      const client = await prisma.client.findFirst({
        where: { id: project.clientId, organizationId: orgId },
      });
      if (client) {
...
export async function assignStaff(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const targetUser = await prisma.user.findFirst({
      where: { id: userId, organizationMembers: { some: { organizationId: orgId } } },
    });
    if (!targetUser) {
      throw new AppError('Target user not found in this organization.', 404);
    }
