...
export async function uploadDivisionMedia(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Upload, configure public sharing, generate URL, and verify accessibility
    let fileId: string;
    let rawUrl: string;
    try {
...
/**
 * Controller: Update existing Division
 */
export async function updateDivision(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      // 1. Delete all old media records in DB in org context
      await tx.divisionMedia.deleteMany({
        where: { division: { id, organizationId: orgId } }
      });
...
export async function deleteDivision(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Delete database record in org context
...
/**
 * Controller: Get published Divisions
 */
export async function getPublicDivisions(_req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const proposedSlug = _req.headers['x-organization-slug'] as string;
    const proposedId = _req.headers['x-organization-id'] as string;
...
/**
 * Controller: Stream Public Division Media directly from Google Drive
 */
