...
export async function getTasks(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (user.role === Role.Client) {
      // Find client profile mapped to email in org context
      ...
      tasks = await prisma.task.findMany({
        where: { clientId: client.id, organizationId: orgId },
        include: { 
          event: true,
          client: true,
          project: {
      ...
      // Staff roles: See tasks assigned to them, or tasks for projects they are assigned to in org context
      const assignments = await prisma.staffAssignment.findMany({
        where: { userId: user.id, project: { organizationId: orgId } },
      });
      ...
      // Admin/Manager: Full access in org context
      tasks = await prisma.task.findMany({
  ...
  try {
    ...
    if (user.role === Role.Client) {
      ...
      const assignment = await prisma.staffAssignment.findFirst({
        where: { userId: user.id, projectId: task.projectId || undefined, project: { organizationId: orgId } },
      });
