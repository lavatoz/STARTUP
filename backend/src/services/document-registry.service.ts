...
export class DocumentRegistryService {
   ...
   */
  static async generateDocumentId(organizationId: string, prefix?: string): Promise<string> {
    try {
      ...
      if (!resolvedPrefix) {
        ...
        resolvedPrefix = defaultCompany?.invoicePrefix || 'STUDIO';
      }

      const cleanPrefix = resolvedPrefix.trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
      const year = new Date().getFullYear();

      // Insert/update in DocumentCounter using organizationId, prefix, DOC prefix and year
      const result = await prisma.$queryRaw<Array<{ lastValue: number }>>`
        ...
        VALUES (${organizationId}, ${cleanPrefix}, 'DOC', ${year}, 1)
  ...
  static async getOrCreateDocumentId(
    ...
    try {
      ...
      const documentId = await this.generateDocumentId(organizationId, prefix);
