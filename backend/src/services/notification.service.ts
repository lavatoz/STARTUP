 ...
 */
export async function createNotification(userId: string, title: string, message: string, organizationId?: string) {
  return NotificationService.emitNotification(userId, { title, message, organizationId });
}
