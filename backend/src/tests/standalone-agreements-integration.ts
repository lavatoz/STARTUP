...
export async function runTests() {
  ...
  for (const model of prismaModels) {
    let mockVal = mockPrisma[model];
    Object.defineProperty(mockPrisma, model, {
      get() {
        if (mockVal && typeof mockVal === 'object') {
          return new Proxy(mockVal, {
            get(target, prop) {
              if (prop === 'findFirst' && !target.findFirst && target.findUnique) {
                return target.findUnique;
              }
  ...
  const testAdminUser = { id: 'admin-1', email: 'admin@apco.com', role: Role.SystemAdmin, organizationId: 'org-id' };
  const testClientUser = { id: 'client-user-1', email: 'client@apco.com', role: Role.Client, organizationId: 'org-id' };
  const testUnrelatedClient = { id: 'client-user-2', email: 'other@apco.com', role: Role.Client, organizationId: 'org-id' };
  const testPhotographer = { id: 'photo-1', email: 'photo@apco.com', role: Role.Photographer, organizationId: 'org-id' };

  const mockClientRecord = { id: 'client-rec-1', name: 'Joel', email: 'client@apco.com', deletedAt: null, organizationId: 'org-id' };
  const mockAgreementRecord = { id: 'agr-1', clientId: 'client-rec-1', templateId: 'tpl-1', title: 'NDA', status: 'PENDING', createdAt: new Date(), organizationId: 'org-id' };
  const mockDocumentRecord = { id: 'doc-1', agreementId: 'agr-1', documentType: DocumentType.AADHAAR, fileUrl: '/api/standalone-agreements/documents/download/file-123.pdf', organizationId: 'org-id' };
