...
function createMockRequestResponse(body = {}, params = {}, query = {}, headers = {}, user?: any) {
  const req = {
    ...
    userContext: user ? {
      userId: user.id,
      organizationId: user.organizationId || 'mock-org-id',
      membershipId: user.membershipId || 'mock-member-id',
...
async function runTests() {
  ...
  const originalDocumentRegistry = mockPrisma.documentRegistry;
  const originalOrganizationMember = mockPrisma.organizationMember;
  const originalEvent = mockPrisma.event;
  const originalGlobalSetting = mockPrisma.globalSetting;
  const originalInvoiceItem = mockPrisma.invoiceItem;
  const originalQuotationItem = mockPrisma.quotationItem;
  const originalExpense = mockPrisma.expense;
  const originalWorkflowEvent = mockPrisma.workflowEvent;
  const originalPersonnel = mockPrisma.personnel;
  const originalPersonnelEventAssignment = mockPrisma.personnelEventAssignment;
  const originalAgreementTemplate = mockPrisma.agreementTemplate;
  ...
  mockPrisma.organizationMember = {
    create: async (args: any) => ({ id: 'org-member-id', ...args.data }),
    findUnique: async () => null,
    findFirst: async () => null,
    update: async (args: any) => ({ id: 'org-member-id', ...args.data }),
    delete: async () => ({}),
  };
  ...
  mockPrisma.documentRegistry = {
    findFirst: async () => null,
    create: async (args: any) => ({ documentId: 'doc-id', verificationUrl: 'http://verify.apco/doc-id', documentNumber: 'DOC-123', documentType: 'INVOICE', sha256Hash: 'hash', ...args.data }),
    upsert: async (args: any) => ({ documentId: 'doc-id', verificationUrl: 'http://verify.apco/doc-id', documentNumber: 'DOC-123', documentType: 'INVOICE', sha256Hash: 'hash', ...args.create }),
  };
  ...
  // Common Mock Data
  const adminUser = { id: 'admin-id', email: 'admin@apco.com', firstName: 'Admin', lastName: 'User', role: Role.SystemAdmin, mfaEnabled: false, mustChangePassword: false, organizationId: 'org-id' };
  const managerUser = { id: 'manager-id', email: 'manager@apco.com', firstName: 'Manager', lastName: 'User', role: Role.Manager, mfaEnabled: false, mustChangePassword: false, organizationId: 'org-id' };
  const photographerUser = { id: 'photographer-id', email: 'photographer@apco.com', firstName: 'Photo', lastName: 'Grapher', role: Role.Photographer, mfaEnabled: false, mustChangePassword: false, organizationId: 'org-id' };
  const clientUser = { id: 'client-user-id', email: 'client@apco.com', firstName: 'Client', lastName: 'User', role: Role.Client, mfaEnabled: false, mustChangePassword: false, organizationId: 'org-id' };

  const mockClientRecord = { id: 'client-rec-id', name: 'Client Corp', email: 'client@apco.com', deletedAt: null, organizationId: 'org-id' };
  const mockProjectRecord = { id: 'project-uuid', name: 'Test Project', status: 'Draft', clientId: 'client-rec-id', deletedAt: null, organizationId: 'org-id' };
  ...
  await testCase('Workflows: Staff assignment logs STAFF_ASSIGNED', async () => {
    ...
    mockPrisma.user.findFirst = async () => photographerUser;
  ...
  await testCase('Workflows: Staff removal logs STAFF_REMOVED', async () => {
    ...
    mockPrisma.project.findFirst = async () => mockProjectRecord;
    mockPrisma.project.findUnique = async () => mockProjectRecord;
    mockPrisma.staffAssignment.findFirst = async () => ({ id: 'assign-id', userId: photographerUser.id, eventIds: [] });
  ...
  await testCase('Settings: Get company by ID', async () => {
    ...
    mockPrisma.companyProfile.findFirst = async () => mockCompany;
  ...
  await testCase('File Operations: Presigned download URL access checking', async () => {
    ...
    mockPrisma.file.findFirst = async () => securedFile;
  ...
  await testCase('Settings: Save company profile', async () => {
    ...
    mockPrisma.organization.update = async (args: any) => ({ id: args.where.id });
  ...
  await testCase('Settings: Delete company profile', async () => {
    ...
    mockPrisma.companyProfile.findFirst = async () => ({ id: 'comp-2', companyName: 'Brand B' });
  ...
  await testCase('Files: Confirm upload', async () => {
    ...
    mockPrisma.file.findFirst = async () => null;
  ...
  await testCase('Files: Soft delete file', async () => {
    ...
    mockPrisma.file.findFirst = async () => ({ id: 'file-2', userId: photographerUser.id, key: 'uploads/f-2.png', originalName: 'f-2.png' });
  ...
  await testCase('Approvals: Verify approval records and cascades', async () => {
    ...
    mockPrisma.approval.findFirst = async () => ({
      id: 'appr-1',
      targetId: 'inv-1',
      targetType: 'invoice',
      clientName: 'Client Corp',
  ...
  await testCase('Events: updateEvent updates event and logs EVENT_UPDATE for Admin/Manager', async () => {
    ...
    mockPrisma.event.findFirst = async () => ({ id: 'event-update-id', name: 'Original Shoot', date: new Date() });
  ...
  await testCase('Events: deleteEvent deletes event and logs EVENT_DELETE for Admin/Manager', async () => {
    ...
    mockPrisma.event.findFirst = async () => ({ id: 'event-del-id', name: 'Shoot to delete' });
  ...
  await testCase('Tasks: updateTask permissions check', async () => {
    ...
    mockPrisma.task.findFirst = async () => mockTask;
  ...
  await testCase('Quotations: Brand resolution - Brand-Specific Fallback Logo (Aaha Kalyanam)', async () => {
    ...
    if (getResults().nextError) throw getResults().nextError;
    if (!passedData) throw new Error('PDF generation was not called.');
    if (passedData.companyLogoUrl) {
      throw new Error(`Expected no hardcoded fallback logo for Aaha Kalyanam, got ${passedData.companyLogoUrl}`);
    }
  });
  ...
  await testCase('Quotations: Brand resolution - Brand-Specific Fallback Logo (Tiny Toes)', async () => {
    ...
    if (!passedData) throw new Error('PDF generation was not called.');
    if (passedData.companyLogoUrl) {
      throw new Error(`Expected no hardcoded fallback logo for Tiny Toes, got ${passedData.companyLogoUrl}`);
    }
  });
  ...
  await testCase('Personnel Event Assignments: Assign personnel to multiple events', async () => {
    ...
    mockPrisma.personnel.findFirst = async (args: any) => {
      if (args.where.id === 'shone-id') {
        return { id: 'shone-id', name: 'Shone User', role: 'Photographer' };
      }
      return null;
  ...
  await testCase('Personnel Event Assignments: Prevent duplicate assignments', async () => {
    ...
    mockPrisma.personnel.findFirst = async () => ({ id: 'shone-id', name: 'Shone User' });
    mockPrisma.event.findUnique = async () => ({ id: 'event-1', name: 'Save the Date' });
    mockPrisma.event.findFirst = async () => ({ id: 'event-1', name: 'Save the Date' });
  ...
  await testCase('Personnel Event Assignments: API formats arrays and returns correct response', async () => {
    ...
    mockPrisma.personnel.findFirst = async () => ({
      id: 'shone-id',
      name: 'Shone User',
      role: 'Photographer',
      assignments: [
  ...
  console.log('------------------------------');
    ...
    mockPrisma.documentRegistry = originalDocumentRegistry;
    mockPrisma.organizationMember = originalOrganizationMember;
    mockPrisma.event = originalEvent;
    mockPrisma.globalSetting = originalGlobalSetting;
    mockPrisma.invoiceItem = originalInvoiceItem;
    mockPrisma.quotationItem = originalQuotationItem;
    mockPrisma.expense = originalExpense;
    mockPrisma.workflowEvent = originalWorkflowEvent;
    mockPrisma.personnel = originalPersonnel;
    mockPrisma.personnelEventAssignment = originalPersonnelEventAssignment;
    mockPrisma.agreementTemplate = originalAgreementTemplate;
  }
