...
model Organization {
  ...
  // Relations to tenant-owned resources
  clients             Client[]
  projects            Project[]
  events              Event[]
  tasks               Task[]
...
for (const modelName of targetModels) {
  ...
  schema = schema.replace(modelRegex, (match, body) => {
    // Check if organizationId is already present (prevent double runs)
    if (body.includes('organizationId')) {
      return match;
    }
    
    // Inject fields and relation at the top of the body
    let newBody = `\\n  organizationId String?\\n  organization   Organization? @relation(fields: [organizationId], references: [id], onDelete: Cascade)\\n` + body;
    
    // Append index if not present
